...
def train_cv(config):    
    ...
    for foldcounter, (train_index, test_index) in enumerate(kf.split(patientID, truelabels)):
        ...
        dls = dblock.dataloaders(combined_df, bs = config["batch_size"])
        ...
        print(f"test trainLoaders - one batch: patch_ids {patch_ids.shape} input {embeds.shape} label {labels.shape}")

        print('test valLoaders...')
        (patch_ids, embeds), labels = next(iter(valLoaders))
        patch_ids = np.array(patch_ids)  
        patch_ids = np.transpose(patch_ids)  
        print(f"test valLoaders - one batch: patch_ids {patch_ids.shape} input {embeds.shape} label {labels.shape}")
