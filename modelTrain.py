...
resFolder = f'/scratch/prj/cb_normalbreast/Siyuan/prj_normal/BreastAgeNet/experiments/KHP_RM/{model_name}_{stainFunc}'
...
dim_input = get_dim_input(model_name)
...
for train_index, test_index in kf.split(patientID, yTrue):
    ...
    train_data, val_data, test_data = split_data(clinic_df, rootdir, patientID, yTrue, train_index, test_index, fold_pt) 
    ...
    model, train_loss_history, train_acc_history, val_loss_history, val_acc_history = train_model(model, trainLoaders, valLoaders, optimizer, criterion, patience, minEpochTrain, max_epochs, model_name, foldcounter,resFolder)
  
    torch.save(model.state_dict(), f'{resFolder}/{model_name}_{stainFunc}_MILfinalModel_fold{foldcounter}')
