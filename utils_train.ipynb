...
#     def __init__(self, n_in, n_heads, n_latent: Optional[int] = None) -> None:
#         super().__init__()
...
#         attention_scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.d_k ** 0.5)  # [batch_size, n_heads, bag_size, bag_size]
#         attention_weights = F.softmax(attention_scores, dim=-1)  # softmax
#         attention_output = attention_weights.mean(dim=-2)  # [batch_size, n_heads, bag_size]
#         attention_output = attention_output.transpose(1, 2).contiguous()  # [batch_size, bag_size, n_heads]
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_in, n_heads, n_latent: Optional[int] = None, dropout: float = 0.1) -> None:
        ...
        self.q_proj = nn.Linear(n_in, n_latent)
        self.k_proj = nn.Linear(n_in, n_latent)
        self.fc = nn.Linear(n_latent, 1)  # Adjusted to aggregate across heads directly
...
#         self.q_proj = nn.Linear(n_in, n_latent)
#         self.k_proj = nn.Linear(n_in, n_latent)
#         self.v_proj = nn.Linear(n_in, n_latent)  # Adding V projection
#         self.fc = nn.Linear(n_latent, 1)  # Adjusting to aggregate all heads directly
#         self.split_heads = lambda x: x.view(x.shape[0], x.shape[1], self.n_heads, self.d_k).transpose(1, 2)

#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         Q = self.q_proj(x)  # Removed tanh
#         K = self.k_proj(x)  # Removed sigmoid
#         V = self.v_proj(x)  # Added V projection
        
#         Q = self.split_heads(Q)  # [batch_size, n_heads, bag_size, d_k]
#         K = self.split_heads(K)  # [batch_size, n_heads, bag_size, d_k]
#         V = self.split_heads(V)  # [batch_size, n_heads, bag_size, d_k]

#         attention_scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.d_k ** 0.5)  # Scaled dot-product
#         attention_weights = F.softmax(attention_scores, dim=-1)  # softmax over bag_size
        
#         attention_output = torch.matmul(attention_weights, V)  # [batch_size, n_heads, bag_size, d_k]
#         attention_output = attention_output.transpose(1, 2).contiguous().view(x.shape[0], x.shape[1], -1)  # Flatten heads
...
class BreastAgeNet(nn.Module):
    def __init__(self, n_feats, attention, n_classes, n_heads=8, n_latent=512):
        ...
        self.classifiers = nn.ModuleList([nn.Sequential(nn.Linear(n_latent, n_latent//2), nn.ReLU(),nn.Dropout(0.5),nn.Linear(n_latent//2, 1)) for _ in range(n_classes)])

    
    
    def forward(self, bags, embed_attn=False):
        bags = bags.to(device)                        # [batch_size, bag_size, n_feats]
        ...
        for i in range(self.n_classes):
            ...
            attention_scores = self.attentions[i](embeddings)  # Shape: [batch_size, bag_size, n_classes]
            attns.append(attention_scores)
            # Apply softmax to the attention scores across the bag_size dimension
            A = F.softmax(attention_scores, dim=1)  # Shape: [batch_size, bag_size, 1]
            # Apply weighted sum for each instance in the bag
            M = torch.bmm(A.transpose(1, 2), embeddings)  # Shape: [batch_size, 1, n_latent]
            M = M.squeeze()  # Shape: [batch_size, n_latent]
            # Pass through classifier to get logits for this class
            logit = self.classifiers[i](M)  # Shape: [batch_size, 1]
            logits.append(logit)
        
        # Stack logits along a new dimension for consistent output shape
        logits = torch.cat(logits, dim=1)  # Shape: [batch_size, n_classes]

        if embed_attn:
            return logits, embeddings, attns
        else:
            return logits
